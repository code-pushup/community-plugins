...
import { suiteResultToAuditOutput, toAuditSlug } from './utils.js';

describe('toAuditSlug', () => {
  it('should create slug string', () => {
    expect(toAuditSlug('glob')).toBe(`${JS_BENCHMARK_PLUGIN_SLUG}-glob`);
  });
});
...
describe('suiteResultToAuditOutput', () => {
  it('should produce valid minimal AuditOutput for a single result', () => {
    ...
    expect(() => auditOutputSchema.parse(auditOutput)).not.toThrowError();
  });
