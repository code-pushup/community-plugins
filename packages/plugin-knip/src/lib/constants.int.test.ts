...
describe('constants-AUDITS', () => {
  it.each(KNIP_AUDITS.map((audit) => [audit.slug, audit]))(
    ...
    (_, audit) => {
      expect(() => auditSchema.parse(audit)).not.toThrowError();
    },
  );
});
...
describe('constants-KNIP_GROUPS', () => {
  it('should be a valid file group info', () => {
    expect(() => groupSchema.parse(KNIP_GROUP_FILES)).not.toThrowError();
  });

  it('should be a valid exports group info', () => {
    expect(() => groupSchema.parse(KNIP_GROUP_EXPORTS)).not.toThrowError();
  });

  it('should be a valid dependencies group info', () => {
    expect(() => groupSchema.parse(KNIP_GROUP_DEPENDENCIES)).not.toThrowError();
  });

  it('should be a valid all group info', () => {
    expect(() => groupSchema.parse(KNIP_GROUP_ALL)).not.toThrowError();
  });
});
