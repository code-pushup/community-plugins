...
describe('lintStyles', () => {
  ...
  it('should use stylelint.lint to generate lint results', async () => {
    ...
    await expect(lintStyles(options)).resolves.not.toThrowError();
  ...
  it('should throw an error if stylelint.lint fails', async () => {
    await expect(lintStyles({})).rejects.toThrowError(
...
describe('lintStylescustom', () => {
  ...
  it.each([['js'], ['mjs'], ['cjs'], ['yml'], ['json']])(
    ...
    async (configFileFormat) => {
      ...
      // Check that the expected warning is present (there may be additional warnings from stylelint-config-standard)
      expect(warnings).toContainEqual(colorNoInvalidHexWarning);
...
describe('lintStyles logic with extends', () => {
  ...
  it('should lint files correctly without extends', async () => {
    ...
    expect(warnings.at(0)!.rule).toBe('block-no-empty');
  });
