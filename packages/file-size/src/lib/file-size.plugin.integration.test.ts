...
import {MEMFS_VOLUME} from 'testing-utils';
import {
    type FileSizePluginConfig,
    audits,
    fileSizePlugin,
    fileSizeCategories,
    pluginSlug as slug,
...
describe('fileSizePlugin', () => {
    const baseOptions: FileSizePluginConfig = {
        directory: '/',
    };
    ...
    it('should return valid PluginConfig', () => {
        const pluginConfig = fileSizePlugin(baseOptions);
    ...
    it('should use pattern', async () => {
        const pluginConfig = fileSizePlugin({
            ...baseOptions,
            pattern: /\.js$/,
        });
...
describe('fileSizeCategories', () => {
    it.each(fileSizeCategories)(
