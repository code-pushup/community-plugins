...
import { normalizePathForMatching, splitPathSegments } from './path-utils.js';
...
export function applyGrouping(
  ...
  if (!groups || groups.length === 0) {
    return [...nodes].toSorted((a, b) => b.values.bytes - a.values.bytes);
  }
  ...
  for (const group of groups) {
    const { title, includeInputs, icon, numSegments: maxDepth } = group;
      ...
      if (!shouldCreateSingleGroup && maxDepth && maxDepth > 0) {
        ...
        pathGroups.forEach((nodesInGroup, groupPath) => {
          ...
          const folderNode: StatsTreeNode = {
            ...
            children: nodesInGroup.toSorted(
        ...
        const groupNode: StatsTreeNode = {
          ...
          children: nodesToGroup.toSorted(
            (a, b) => b.values.bytes - a.values.bytes,
          ),
        };
