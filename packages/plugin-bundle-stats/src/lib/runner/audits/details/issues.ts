...
export function checkBlacklistIssues(
  ...
  if (output.inputs) {
    for (const inputPath in output.inputs) {
      if (!Object.prototype.hasOwnProperty.call(output.inputs, inputPath)) {
        continue;
      }
...
export function getIssues(
  ...
  for (const outputPath in statsSlice) {
    if (!Object.prototype.hasOwnProperty.call(statsSlice, outputPath)) {
      continue;
    }
...
type BlacklistedPatternIssueOptions = {
...
export function createBlacklistedPatternIssue(
  options: BlacklistedPatternIssueOptions,
): Issue {
  const { summaryMessage, hint } = options;
  ...
  return {
    message,
    severity: 'error',
    source: { file: options.outputPath },
  };
}
