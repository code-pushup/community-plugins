...
/**
 * Validates basic structure of bundle stats data.
 */
function validateStatsStructure(
  stats: unknown,
  artefactsPath: string,
): asserts stats is Record<string, unknown> {
  if (!stats) {
...
/**
 * Validates bundler-specific properties in stats data.
 */
function validateBundlerSpecificProperties(
  ...
  switch (bundler) {
    ...
    case 'rsbuild':
      if (
        ...
        throw new Error(
          `Bundle stats file missing required ${bundler} properties (assets, chunks, modules): ${artefactsPath}`,
        );
      }
      break;
    case 'vite':
...
export function validateBundleStats(
  ...
  validateStatsStructure(stats, artefactsPath);
  validateBundlerSpecificProperties(stats, artefactsPath, bundler);
}

type BundlerStats = Record<string, unknown>;
type UnifyOptions = Record<string, unknown>;
...
export function getUnifyFunction(
  bundler: SupportedBundlers,
): (stats: BundlerStats, options?: UnifyOptions) => UnifiedStats {
  switch (bundler) {
    case 'esbuild':
      return unifyEsbuildStats as (
        stats: BundlerStats,
        options?: UnifyOptions,
      ) => UnifiedStats;
    case 'webpack':
      return unifyWebpackStats as (
        stats: BundlerStats,
        options?: UnifyOptions,
      ) => UnifiedStats;
    case 'rsbuild':
      return unifyRsbuildStats as (
        stats: BundlerStats,
        options?: UnifyOptions,
      ) => UnifiedStats;
    case 'vite':
      return unifyViteStats as (
        stats: BundlerStats,
        options?: UnifyOptions,
      ) => UnifiedStats;
    case 'sonda':
      return unifySondaStats as (
        stats: BundlerStats,
        options?: UnifyOptions,
      ) => UnifiedStats;
    default:
...
export function mergeDependencyTreeConfig(
  ...
  if (pluginOptions) {
    return {
      ...
      pruning: {
        maxDepth: 2,
        maxChildren: 20, // Default max children for dependency tree
        pathLength: 60,
...
export async function bundleStatsRunner(
  ...
  return async () => {
    ...
    const unifiedBundleStats = unifyBundlerStats(stats as BundlerStats, {});
