...
export async function readReportJson(baseDir: string, outputPath = '.code-pushup', filename = 'report.json'): Promise<Report> {
    return readJsonFile(join(baseDir, outputPath, filename));
}

export async function executeCliCollectForPlugin(pluginSlug: string, args: Omit<CoreConfig, 'plugins' | 'categories'>, cwd: string): Promise<ProcessResult> {
    return executeCli('collect', {
        onlyPlugins: [pluginSlug],
    }, cwd);
}

export async function executeCli(command: 'autorun' | 'collect' | 'upload', args: Omit<CoreConfig, 'plugins' | 'categories'> & {
    onlyPlugins?: string[]
}, cwd: string): Promise<ProcessResult> {
    return executeProcess({
        command: 'code-pushup',
        args: [command, ...objectToCliArgs({
            progress: false,
            ...args
        })],
        ...(cwd ? {cwd} : {})
...
export async function addCodePushupConfigFile(options?: {
    ...
    const {format = 'ts', config, targetFolder = '', fileImports, fileName, pluginStrings} = options ?? {};

    const filePath = resolve(targetFolder, `${fileName ?? 'code-pushup.config'}.${format}`);
    try {
...
export function getRelativePathToPluginDist(baseDir: string, pluginName: string) {
